...
const HabitRow = ({ habit, weekDates, onToggle, onEdit, onDelete, trackerStartDate, isEditableInTracker, isColorCoded, isRowColored, isLastHabitRow, isMonthlyView }) => {
      ...
      <td className={`py-4 font-medium border border-[#333333] ${isColorCoded && isRowColored ? bgClassMap[habit.color] : "bg-[#121212]"} ${isLastHabitRow ? "rounded-bl-lg" : ""}`}>
        <div className="flex items-center justify-between pl-3">
          <div className="flex items-center gap-2">
            ...
            <div className={`min-w-[6rem] lg:w-full truncate ${isMonthlyView ? "max-w-[10rem]" : "max-w-[15rem]"}`}>{habit.name}</div>
          </div>
